...
const { t } = useI18n()
const imagesApi = inject('services')?.get('images.api')
...
watch(() => props.file, (files) => {
  ...
  rotation.value = 0
  if (files[0] && files[1]) {
    props.winManager?.setTitle(props.winId, `${files[0].name} \u2194 ${files[1].name}`)
  }
}, { immediate: true })
...
function reset()   { transform.scale = 1; transform.x = 0; transform.y = 0 }
...
function makeImgStyle(base) {
  return {
    transform: `translate(${transform.x}px, ${transform.y}px) scale(${base * transform.scale}) rotate(${rotation.value}deg)`,
...
<template>
  <div
    ...
    @touchend="onTouchEnd"
    @dblclick="e => { if (!e.target.closest('.viewer-toolbar')) reset() }"
  >
