...
- **图片** — 缩略图网格、原图平移/缩放查看器、左右对比滑动查看器；**图片编辑器**：多图层管理、15 种绘制/选择工具（移动、画笔、橡皮擦、裁剪、填充、渐变、文字、形状、套索、魔棒、模糊、涂抹等）、色调调整（亮度/对比度、曲线、色阶、色相/饱和度、色彩平衡、曝光/自然饱和度、阴影/高光）、WebGL 加速滤镜（高斯模糊、锐化、噪点、暗角、像素化、怀旧、浮雕、边缘检测、色差等）、撤销/重做历史、导出（可选格式与质量）；**PSD** 文件支持完整图层查看
- **表格数据** — Parquet、CSV、JSON、JSONL，基于 Polars，支持 SQL `WHERE` 过滤、排序、Schema 浏览、图片列预览及内联图片编辑
- **压缩包** — 浏览 zip、tar、tar.gz、tar.bz2、tar.xz、7z；随机访问预览（zip/7z）；解压到当前目录或子文件夹；创建压缩包（可设压缩级别和密码）
- **MIDI** — 基于 Web Audio 合成播放、示波器可视化、增益控制、循环模式、下载
- **视频与音频** — HTTP 范围请求流式播放
...
### 其他
- **书签** — 收藏常用目录，通过侧边栏快速访问（保存在 localStorage）
...
**后端：** Python 3.10+、FastAPI、Uvicorn、Polars、Pillow、py7zr
