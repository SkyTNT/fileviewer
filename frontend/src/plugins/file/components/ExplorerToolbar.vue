...
import { useAppTheme, ACCENT_COLORS } from '@/plugins/theme/index.js'
...
// ── mkdir / touch / paste (via write store) ───────────────────────────────────
...
<template>
  ...
  <template v-if="!mobile">
    ...
    <template v-if="store.writeMode && !store.isAtHome">
      ...
      <v-btn icon size="small" class="mr-1" @click="writeStore.openTouch">
        <v-icon size="20">mdi-file-plus-outline</v-icon>
        <v-tooltip activator="parent">{{ t('toolbar.newFile') }}</v-tooltip>
      </v-btn>
      <v-btn icon size="small" class="mr-1" @click="writeStore.openMkdir">
