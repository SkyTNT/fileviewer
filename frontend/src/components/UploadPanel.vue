...
const uploading = computed(() => store.uploadTasks.filter(t => t.status === 'uploading').length)
const done      = computed(() => store.uploadTasks.filter(t => t.status === 'done').length)
const errors    = computed(() => store.uploadTasks.filter(t => t.status === 'error').length)
...
// Auto-expand whenever a new upload is added
watch(() => store.uploadTasks.length, (n, prev) => { if (n > prev) collapsed.value = false })
    ...
    <div v-if="store.uploadTasks.length" class="upload-panel">
      <v-card elevation="8" rounded="lg" width="320">
        ...
        <div class="d-flex align-center px-3 py-2" style="cursor:pointer; min-height:44px" @click="collapsed = !collapsed">
          ...
          <v-btn icon size="x-small" variant="text" :disabled="uploading > 0" @click.stop="store.clearAllUploads">
        ...
        <template v-if="!collapsed">
          ...
          <template v-if="hasDone">
            ...
            <div class="px-2 py-1 text-right">
              <v-btn size="x-small" variant="text" color="medium-emphasis" @click="store.clearUploadsDone">
                {{ t('upload.clearDone') }}
              </v-btn>
            </div>
          </template>
