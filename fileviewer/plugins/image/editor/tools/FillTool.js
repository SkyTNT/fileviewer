...
function floodFillCanvas(canvas, sx, sy, fillR, fillG, fillB, fillA, tolerance, mask) {
  ...
  while (queue.length > 0) {
    ...
    data[pi] = fillR; data[pi+1] = fillG; data[pi+2] = fillB; data[pi+3] = fillA
...
export default {
  ...
  onPointerDown(e, toolCtx) {
    ...
    const { r, g, b, a } = hexToRgb(state.fgColor)
    const mask = state.selection ? selectionToMask(state.selection, state.canvasWidth, state.canvasHeight) : null
    floodFillCanvas(layer.canvas, e.x, e.y, r, g, b, a, state.fillTolerance, mask)
    pushHistory('Fill')
    state.isDirty = true
    invalidate()
  },
